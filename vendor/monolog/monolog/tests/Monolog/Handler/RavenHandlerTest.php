...
class RavenHandlerTest extends TestCase
{
    ...
    public function testUserContext()
    {
        ...
        $recordWithNoContext = $this->getRecord(Logger::INFO, 'test with default users context');
        // set users context 'externally'
        ...
        $recordWithContext = $this->getRecord(Logger::INFO, 'test', array('users' => $user));

        $ravenClient->user_context(array('id' => 'test_user_id'));
        // handle context
        $handler->handle($recordWithContext);
        $this->assertEquals($user, $ravenClient->lastData['users']);
