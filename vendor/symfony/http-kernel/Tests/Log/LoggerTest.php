...
class LoggerTest extends TestCase
{
    ...
    {
        $this->logger->{$level}($message, array('users' => 'Bob'));
        $this->logger->log($level, $message, array('users' => 'Bob'));
    ...
    {
        return array(
            LogLevel::EMERGENCY => array(LogLevel::EMERGENCY, 'message of level emergency with context: {users}'),
            LogLevel::ALERT => array(LogLevel::ALERT, 'message of level alert with context: {users}'),
            LogLevel::CRITICAL => array(LogLevel::CRITICAL, 'message of level critical with context: {users}'),
            LogLevel::ERROR => array(LogLevel::ERROR, 'message of level error with context: {users}'),
            LogLevel::WARNING => array(LogLevel::WARNING, 'message of level warning with context: {users}'),
            LogLevel::NOTICE => array(LogLevel::NOTICE, 'message of level notice with context: {users}'),
            LogLevel::INFO => array(LogLevel::INFO, 'message of level info with context: {users}'),
            LogLevel::DEBUG => array(LogLevel::DEBUG, 'message of level debug with context: {users}'),
        );
    }
    ...
    public function testLogLevelDisabled()
    {
        ...
        $this->logger->debug('test', array('users' => 'Bob'));
        $this->logger->log(LogLevel::DEBUG, 'test', array('users' => 'Bob'));
    ...
    {
        $logger = $this->logger;
        $logger->info('{Message {nothing} {users} {foo.bar} a}', array('users' => 'Bob', 'foo.bar' => 'Bar'));
